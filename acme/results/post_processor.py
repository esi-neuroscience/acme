...
class ResultPostProcessor:
    ...
    def __init__(
        ...
        self.write_worker_results = write_worker_results
    ...
    def process_futures(
        ...
        # Assume a user-defined saving scheme
        # (`write_worker_results` = `collect_results` = False)
        results = None
    ...
    def _handle_normal_file_results(
        ...
        if stacking_dim is not None:
            self._add_missing_return_values(values, payloadDir)

        self.finalMsg = self.finalMsg.format(self.results_container)
        msg = "Container ready, links to data payload located in %s"
        log.debug(msg, payloadDir)
        log.debug("Returning a list of file-names")
        return values

    def _add_missing_return_values(self, values: List[str], payloadDir: str) -> None:
