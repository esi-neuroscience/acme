...
from acme.argument_processor import ArgumentProcessor, ArgumentProcessingError
...
class TestArgumentProcessor:
    ...
    def test_dryrun_setup_basic(self):
        ...
        # Call method
        indices, args, kwargs = ArgumentProcessor.dryrun_setup(argv, kwargv, n_calls, n_runs=2)
    ...
    def test_dryrun_setup_with_single_elements(self):
        ...
        # Call method
        indices, args, kwargs = ArgumentProcessor.dryrun_setup(argv, kwargv, n_calls, n_runs=1)
    ...
    def test_dryrun_setup_auto_n_runs(self):
        ...
        for n_calls, expected_min_runs in test_cases:
            ...
            indices, args, kwargs = ArgumentProcessor.dryrun_setup(argv, kwargv, n_calls, n_runs=None)
    ...
    def test_dryrun_setup_edge_cases(self):
        ...
        indices, args, kwargs = ArgumentProcessor.dryrun_setup(argv, kwargv, n_calls, n_runs=0)
        assert len(indices) == 0
        assert len(args) == 0
        assert len(kwargs) == 0

        # Test with n_runs > n_calls
        indices, args, kwargs = ArgumentProcessor.dryrun_setup(argv, kwargv, n_calls, n_runs=10)
    ...
    def test_broadcast_arguments_basic(self, mock_sized):
        ...
        # Call method
        new_argv, new_kwargv = ArgumentProcessor.broadcast_arguments(
            argv, kwargv, n_calls, mock_client, MagicMock()
        )
    ...
    def test_format_kwarg_list_basic(self):
        ...
        # Call method
        kwarg_list = ArgumentProcessor.format_kwarg_list(kwargv, n_calls)
...
class TestArgumentProcessingError:
    """Test suite for ArgumentProcessingError exception"""
    
    def test_exception_type(self):
        """Test that ArgumentProcessingError is a proper exception"""
        with pytest.raises(ArgumentProcessingError):
            raise ArgumentProcessingError("Test error")
        
        # Test inheritance
        assert issubclass(ArgumentProcessingError, Exception)
...
class TestIntegration:
    ...
    def test_full_workflow(self):
        ...
        # Step 1: Dryrun setup
        indices, dryrun_args, dryrun_kwargs = ArgumentProcessor.dryrun_setup(
            original_argv, original_kwargv, n_calls, n_runs=2
        )
        ...
        with patch('acme.argument_processor.collections.abc.Sized') as mock_sized:
            mock_sized.return_value = True
            broadcasted_argv, broadcasted_kwargv = ArgumentProcessor.broadcast_arguments(
                original_argv, original_kwargv, n_calls, mock_client, MagicMock()
            )
        
        # Verify broadcasting
        assert len(broadcasted_argv[0]) == 3
        # Should be the future (or future[0] if it was Sized)
        assert broadcasted_argv[0][0] == mock_future or broadcasted_argv[0][0] == mock_future[0]
        
        # Step 3: Format kwarg list
        kwarg_list = ArgumentProcessor.format_kwarg_list(broadcasted_kwargv, n_calls)
