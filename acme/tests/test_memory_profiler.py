...
class TestMemoryProfiler:
    ...
    def test_initialization(self):
        ...
        func = lambda x: x**2
        profiler = MemoryProfiler(ArgumentProcessor([], {}, 1), func, func.__name__)

        assert profiler.func == func
        assert (
            profiler.tqdm_format
    ...
    def test_initialization_with_custom_format(self):
        ...
        profiler = MemoryProfiler(
            ArgumentProcessor([], {}, 1), func, func.__name__, custom_format
        )
    ...
    def test_estimate_memory_basic(
        self, mock_sleep, mock_tqdm, mock_psutil, mock_process
    ):
        ...
        n_calls = 5
        profiler = MemoryProfiler(
            ArgumentProcessor([], {}, n_calls), func, func.__name__
        )
        ...
        # Test the estimation
        result = profiler.estimate_memory(None, 2)
        ...
        assert mock_proc_instance.start.call_count == n_calls
        assert mock_proc_instance.kill.call_count == n_calls

    @patch("acme.memory_profiler.multiprocessing.Process")
    @patch("acme.memory_profiler.psutil.Process")
    @patch("acme.memory_profiler.tqdm.tqdm")
    @patch("acme.memory_profiler.time.sleep")
    def test_estimate_memory_with_output_dir(
        self, mock_sleep, mock_tqdm, mock_psutil, mock_process
    ):
        ...
        result = profiler.estimate_memory("/tmp/output", 2)
        ...
        assert (
            len(mock_process.call_args_list) == n_calls
        )  # Should have `n_calls` calls for `n_calls` jobs

    @patch("acme.memory_profiler.multiprocessing.Process")
    @patch("acme.memory_profiler.psutil.Process")
    @patch("acme.memory_profiler.tqdm.tqdm")
    @patch("acme.memory_profiler.time.sleep")
    def test_estimate_memory_with_varying_memory(
        self, mock_sleep, mock_tqdm, mock_psutil, mock_process
    ):
        ...
        # Test the estimation
        result = profiler.estimate_memory(None, 3)
            ...
            with patch("acme.memory_profiler.psutil.Process") as mock_psutil:
                with patch("acme.memory_profiler.tqdm.tqdm") as mock_tqdm:
                    with patch("acme.memory_profiler.time.sleep") as mock_sleep:
                        ...
                        mock_psutil_instance = MagicMock()
                        mock_psutil_instance.memory_info.return_value.rss = (
                            2 * 1024 * 1024 * 1024
                        )  # 2 GB
                        mock_psutil.return_value = mock_psutil_instance
