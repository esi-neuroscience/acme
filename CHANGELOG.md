...
### NEW
- Check for existing directories auto-generated by ACME (`ACME_<date>_<time>_`)
  and offer to delete either all or just those older than `N` days. Can be 
  actively triggered using `ParallelMap`'s new keyword `cleanup_threshold_days`
- Added `AGENTS.md` instructions for AI coding agents + `OpenCode` configuration 
  compatible with GWDG SAIA

### CHANGED
- Overhauled and fundamentally refactored ACME's backend: the `ACMEdaemon` class 
  shrunk from almost 1200 loc to less than 450; its core components have been 
  migrated to dedicated helper classes that permit easier modifications/extensions

### REMOVED
- Cleaned up filtering for dask-jobqueue warning (not needed any more)

### FIXED
- Repaired rendering of query message in `user_input` to correctly account for 
  missing white spaces
- Hardened testing pipeline to be more robust (especially all `test_existing_cluster`)
  tests; fixed `tox` setup
